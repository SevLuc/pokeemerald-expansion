...
4. Rival pick - 3 forced options (no custom entry)

## Author welcome draft (PENDING GAME NAME)
Casual, old-school, no em dashes, "^^" dropped (won't render in gen-3 font):

  "Hi there! Welcome to {GAME NAME}."
...
## Rivals (locked names): Buhrito · Pchal · Twitch
...
- "Twitch" (6 chars) chosen over "Twitch Chat" (11) to fit the 7-char name cap;
  all three names now fit the normal name system.
...
- Phase 1 (system + intro): forced 3-option pick that stores a choice var
  ...
  Green/Gary/Kaz/Toru) - swap to the 3 streamers + add their gNameChoice_*
...
- GAME NAME: still UNSET (also the CLAUDE.md title placeholder).
