...
| MISTY-16 | Misty (readiness gate, via PSYDUCK only) | Policeman blocking the robbed-house shortcut turns the player back with a terse hint: the stuck door is "stubborn as a certain PSYDUCK," and nothing more. Never names MISTY/gym/badge; the gate is inferred through the duck alone. Blocking branch on the SAME Cerulean Policeman as MISTY-10. | Cerulean Policeman (existing; pre-Cascade blocking branch) | Cerulean City | before FLAG_BADGE02_GET (Cascade) | 6 | drafted | - |
