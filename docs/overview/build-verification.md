...
1. `[x] builds` - compiles clean with no new warnings/errors.
2. `[ ] playtested` - the in-game behavior below actually works.

Check items off (or delete the row) once verified. Newest PR first.

## Build status (2026-08-19, local Mac)
Full `make firered -j15` compiles **clean** (ROM 80.78% of 32MB). All `builds`
boxes below are checked. **Playtesting is still pending** for every PR.

Toolchain notes for future local builds: devkitARM at `/opt/devkitpro`; libpng is
via Homebrew, so `/opt/homebrew/bin` must precede devkitpro's `pkg-config` in PATH
and `PKG_CONFIG_PATH=/opt/homebrew/lib/pkgconfig` must be set, or `gbagfx`/`mapjson`
fail to compile (Error 127 cascade).

Three PR #9 text bugs fixed to make it compile (chars absent from `charmap.txt`):
- `PokemonMansion_3F_Frlg` + `PokemonMansion_B1F_Frlg`: `[..]` margin notes -> `(..)`.
- `CeruleanCity_Gym_Frlg`: Misty's `*POP!*` -> `POP!` (SE still fires).
...
- `[x] builds` - text/flavor reflavors + appended `.string` lines only, no new objects
...
- `[x] builds` - new `IsHMUnlocked()` gate, per-move `hmItemId`, `checkfieldmove`
...
- `[x] builds` - Sabrina's possessed gym adds forced-fight/false-direction script
...
- `[x] builds` - new functions `LevelUpMonFromMenu`, `CursorCb_Level/LevelUp/LevelToCap`;
...
- `[x] builds` - new `ShouldAutoHealPartyAfterBattle()` + `HealPlayerParty()` calls
...
- `[x] builds` - also fixed a previously malformed `B_EV_CAP_TYPE`/`B_EV_CAP_VARIABLE`
...
- `[x] builds` - Pewter Gym gains a real `ON_FRAME` map script (phone-ringing:
