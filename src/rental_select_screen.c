...
// Rental draft: this file is a copy of the Battle Factory select screen, adapted to
// offer RENTAL mons and let the player draft 6 of 12 (the stock screen offers 6 and
// picks FRONTIER_PARTY_SIZE = 3). The engine-global FRONTIER_PARTY_SIZE must not
// change, so a screen-local pick count is used instead.
#define SELECTABLE_MONS_COUNT 12  // offered at the draft (was 6)
#define SELECT_PICK_COUNT     6   // the player keeps 6 (was FRONTIER_PARTY_SIZE = 3)
...
static u8 (*sRentalSelect_CurrentOptionFunc)(void) = NULL; // file-local (was COMMON_DATA global; renamed to avoid clashing with the Factory's)
...
// Entry point for the rental draft (renamed from DoBattleFactorySelectScreen so it
// does not clash with the real Factory's screen, which still lives in
// battle_factory_screen.c). Declared in rental_mode.h.
...
static void Select_InitAllSprites(void)
{
    ...
    for (i = 0; i < SELECTABLE_MONS_COUNT; i++)
    {
        // 12 balls in one tighter row (was 6 at 35px spacing). The cursor arrow reads
        // the ball's x below, so this single change repositions both. Balls overlap at
        // this spacing; a two-row layout can be a later polish pass.
        sFactorySelectScreen->mons[i].ballSpriteId = CreateSprite(&sSpriteTemplate_Select_Pokeball, (19 * i) + 16, 64, 1);
...
static void Select_HandleMonSelectionChange(void)
{
    ...
    if (sFactorySelectScreen->mons[cursorPos].selectedId) // Deselect a mon.
    {
        ...
        if (sFactorySelectScreen->selectingMonsState == SELECT_PICK_COUNT
         && sFactorySelectScreen->mons[cursorPos].selectedId == 1)
        {
            for (i = 0; i < SELECTABLE_MONS_COUNT; i++)
            {
                if (sFactorySelectScreen->mons[i].selectedId == SELECT_PICK_COUNT - 1)
                    break;
            }
...
// Rental version: build the 12 offered mons from the rental pool (gRentalMons) using
// the draft offer rolled by GenerateRentalOffer (gRentalRun.offer). Flat Lv50; the
// per-set IVs stored in each gRentalMons entry override the fixed IV passed here, so
// Hidden Power sets keep their real type.
static void CreateFrontierFactorySelectableMons(u8 firstMonId)
{
    u8 i;
    u32 otId = READ_OTID_FROM_SAVE;

    gFacilityTrainerMons = gRentalMons;
    for (i = 0; i < SELECTABLE_MONS_COUNT; i++)
    {
        u16 monId = gRentalRun.offer[i];
        sFactorySelectScreen->mons[i + firstMonId].monId = monId;
        CreateFacilityMon(&gRentalMons[monId], FRONTIER_MAX_LEVEL_50, 31, otId,
                FLAG_FRONTIER_MON_FACTORY, &sFactorySelectScreen->mons[i + firstMonId].monData);
    }
}
...
// Rental version: write the 6 drafted mons into the player party and record their
// gRentalMons indices in gRentalRun.roster (our own in-RAM storage, since the stock
// frontier rentalMons array only holds 3+3 slots). selectedId is the 1-based pick
// order, so slot i gets the mon whose selectedId == i + 1.
static void Select_CopyMonsToPlayerParty(void)
{
    u8 i, j;

    for (i = 0; i < SELECT_PICK_COUNT; i++)
    {
        for (j = 0; j < SELECTABLE_MONS_COUNT; j++)
        {
            if (sFactorySelectScreen->mons[j].selectedId == i + 1)
            {
                ...
                gRentalRun.roster[i] = sFactorySelectScreen->mons[j].monId;
                break;
            }
        }
    }
    gRentalRun.rosterCount = SELECT_PICK_COUNT;
...
static const u8 sText_ChooseMonsCount[] = _("Draft {STR_VAR_1}/{STR_VAR_2}. Choose one.");
static const u8 sText_RentTheseMons[] = _("Rent these POKéMON?");
...
static void Select_PrintSelectMonString(void)
{
    ...
    // Picks go up to 6 now, so drop the "first/second/third" ordinals for a generic
    // prompt plus a running count, and a confirm line once 6 are chosen.
    if (sFactorySelectScreen->selectingMonsState > SELECT_PICK_COUNT)
    {
        str = sText_RentTheseMons;
    }
    else
    {
        ConvertIntToDecimalStringN(gStringVar1, sFactorySelectScreen->selectingMonsState - 1, STR_CONV_MODE_LEFT_ALIGN, 1);
        ConvertIntToDecimalStringN(gStringVar2, SELECT_PICK_COUNT, STR_CONV_MODE_LEFT_ALIGN, 1);
        StringExpandPlaceholders(gStringVar4, sText_ChooseMonsCount);
        str = gStringVar4;
    }
...
static u8 Select_RunMenuOptionFunc(void)
{
    sRentalSelect_CurrentOptionFunc = sSelect_MenuOptionFuncs[sFactorySelectScreen->menuCursorPos];
    return sRentalSelect_CurrentOptionFunc();
}
...
// Rental draft note: this stock confirm shows exactly 3 mon pictures. Since the
// player now drafts 6, it displays the first 3 of them (selectedId 1..3) above the
// "Rent these POKéMON?" prompt. Kept as-is for a low-risk first cut (a full 6-pic
// confirm is a later polish pass); the yes/no still applies to the whole 6.
static void Select_ShowChosenMons(void)
{
    sFactorySelectScreen->monPics[1].bgSpriteId = CreateSprite(&sSpriteTemplate_Select_MonPicBgAnim, 120, 64, 1);
...
// Renamed to avoid clashing with the real Factory's swap screen. Not used yet;
// kept for the M6 post-battle recruit slice (and so this file has no unused-static
// warnings). Do not call it until it is adapted for rental recruiting.
void DoRentalSwapScreen_Unused(void)
{
